...
const ExpensesEmployee = () => {
  ...
  const handleAddUser = async () => {
    ...
    try {
      const res = await api.post("/users/add", { name: newUser, role: "employee" });
      ...
      <div className="flex flex-col gap-4 sm:gap-6 sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage employee expenses and users
          </p>
        </div>

        <Button variant="contained" onClick={() => setOpenModal(true)}>Add Employee User</Button>
      ...
      {openModel && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md mx-4">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Add New Employee User</h2>
