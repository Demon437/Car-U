...
const Dealer = () => {
  ...
  /* ================= NORMALIZER ================= */
  const normalizeCar = (data: any): DealerCar => {
  ...
  const fetchDealerCars = async () => {
    try {
      ...
      const dealerCars = (res.data || [])
        .filter((car: any) => car.seller?.type === "dealer")
        .map(normalizeCar); // ✅ FLATTEN HERE

      setCars(dealerCars);
    } catch (err) {
      console.error(err);
      setError("Failed to load dealer cars");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-gray-600">Loading dealer cars...</span>
      </div>
    );
  }
    ...
    <div className="p-6 bg-gray-100/60 min-h-screen">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Dealer Cars</h1>
            <p className="text-gray-500 mt-1">
              Cars added by dealer sellers
            </p>
          </div>
